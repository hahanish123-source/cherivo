...
type Block = {
  ...
  letterSpacing: number; radius: number; cardColor: string; cardOpacity: number; image?: string; images?: string[]; imageOpacity: number; galleryLayout?: string; visible: boolean; items?: ReasonItem[];
};
...
export default function CreatePage(){
  ...
  const [publishTitle,setPublishTitle]=useState("A Cherivo moment");
  ...
  useEffect(()=>{
    const saved=localStorage.getItem("cherivo-project"); if(!saved)return;
    try{
      ...
      if(x.audioName)setAudioName(x.audioName); if(x.audioUrl)setAudioUrl(x.audioUrl); if(x.customBg)setCustomBg(x.customBg); if(x.customBgOpacity!==undefined)setCustomBgOpacity(Number(x.customBgOpacity));
  ...
  function projectData(){return {blocks:blocks.map(normalizeBlock),theme,background,globalFont,globalTextColor,globalCardOpacity,globalRadius,globalSpacing,globalMotion,audioName,audioUrl,customBg,customBgOpacity,backgroundBaseColor,bgColor1,bgColor2,bgColor3,bgColor4,backgroundOverlay}}
  function save(){localStorage.setItem("cherivo-project",JSON.stringify(projectData()));notify("Draft saved ✨")}
  ...
  function pickImage(e:ChangeEvent<HTMLInputElement>,setter:(url:string)=>void,max=5_000_000){const f=e.target.files?.[0];if(!f)return;if(f.size>max)return notify(`Use a file under ${Math.round(max/1_000_000)}MB`);const r=new FileReader();r.onload=()=>setter(String(r.result));r.readAsDataURL(f)}
  ...
  return <main className="creator">
    ...
    <div className="creatorGrid">
      ...
      <section className={`preview bg-${background} theme-${theme} motion-${globalMotion}`} style={{"--card-opacity":globalCardOpacity/100,"--page-bg":backgroundBaseColor,"--bg1":bgColor1,"--bg2":bgColor2,"--bg3":bgColor3,"--bg4":bgColor4,"--bg-overlay":backgroundOverlay/100} as CSSProperties}>
        {customBg&&<><div className="customBgLayer" style={{backgroundImage:`url("${customBg}")`,opacity:customBgOpacity/100}}/><div className="backgroundOverlayLayer" style={{opacity:backgroundOverlay/100}}/></>}<div className="previewHint">Tap, swipe or use the button to continue ✨</div><div className="progress">{visible.map((b,i)=><i className={i<=scene?"on":""} key={b.id}/>)}</div>{shown?sectionView(shown):<div className="sceneInner"><h1>No visible sections</h1></div>}</section>
      {!previewOnly&&<aside className="sidePanel editor"><div className="editorHead"><div><h2>Edit anything</h2><small>Selected: {current.title}</small></div><span>{selected+1}/{blocks.length}</span></div>
        ...
        <div className="divider">🎵 Optional music</div><p className="helperText">Only add it if you want music. The recipient gets a play/pause control.</p><label>Choose audio<input type="file" accept="audio/*" onChange={e=>{const f=e.target.files?.[0];if(!f)return;if(f.size>12_000_000)return notify("Use audio under 12MB");const r=new FileReader();r.onload=()=>{setAudioName(f.name);setAudioUrl(String(r.result))};r.readAsDataURL(f)}}/></label>{audioUrl&&<div className="audioControlCard"><b>🎵 {audioName}</b><audio controls src={audioUrl}/><button type="button" className="btn small" onClick={()=>{setAudioName("");setAudioUrl("")}}>Remove music</button></div>}
    ...
    {publishOpen&&<div className="modal publishModal" onClick={()=>{if(!publishing && publishedLink)setPublishOpen(false)}}><div className="modalCard publishCard" onClick={e=>e.stopPropagation()}><div className="modalTop"><div><h2>{publishedLink?"Your private greeting is ready 🔐":"Create a private greeting link"}</h2><p>{publishedLink?"Only someone with this link can open the greeting.":"Give your greeting a private title, then create its unique link."}</p></div><button type="button" onClick={()=>setPublishOpen(false)} aria-label="Close"><X/></button></div>{!publishedLink?<><label>Greeting title<input value={publishTitle} onChange={e=>setPublishTitle(e.target.value)} placeholder="A birthday moment"/></label>
          ...
          <button type="button" className="btn primary full" onClick={publishGreeting} disabled={publishing}>{publishing?"Creating private link…":"Create private link 🔐"}</button></>:<><div className="publishedLinkBox"><span>{publishedLink}</span><button type="button" className="btn" onClick={async()=>{await navigator.clipboard?.writeText(publishedLink).catch(()=>{});notify("Link copied 🔗")}}>Copy link</button></div><button type="button" className="btn primary full" onClick={()=>window.open(publishedLink,"_blank","noopener,noreferrer")}>Open greeting ↗</button><button type="button" className="btn full" onClick={()=>{setPublishedLink("");setPublishOpen(false)}}>Done</button></>}</div></div>}
