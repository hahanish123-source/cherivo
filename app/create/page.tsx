...
type ImageAdjustment = { scale: number; x: number; y: number };
...
type Block = {
  ...
  letterSpacing: number; radius: number; cardColor: string; cardOpacity: number; letterColor?: string; letterSize?: number; letterLineHeight?: number; letterAlign?: "left"|"center"|"right"; image?: string; images?: string[]; imageAdjustments?: Record<string, ImageAdjustment>; imageOpacity: number; audioName?: string; audioUrl?: MediaValue; memoryVideo?: MediaValue; galleryLayout?: string; visible: boolean; items?: ReasonItem[];
};
...
function normalizeBlock(raw:any):Block{
  ...
  return {
    ...
    letterSpacing:Number(raw?.letterSpacing ?? 0), radius:Number(raw?.radius ?? 21), cardColor:raw?.cardColor ?? "#ffffff", cardOpacity:Number(raw?.cardOpacity ?? 8), letterColor:raw?.letterColor ?? "#2d2024", letterSize:Number(raw?.letterSize ?? 17), letterLineHeight:Number(raw?.letterLineHeight ?? 1.8), letterAlign:raw?.letterAlign ?? "left", imageOpacity:Number(raw?.imageOpacity ?? 100), imageAdjustments:raw?.imageAdjustments && typeof raw.imageAdjustments === "object" ? raw.imageAdjustments : {}, audioName:raw?.audioName ?? "", audioUrl:raw?.audioUrl, galleryLayout:raw?.galleryLayout ?? "collage", image:raw?.image ?? "", images:Array.isArray(raw?.images) ? raw.images.filter((x:any)=>typeof x === "string") : (raw?.image ? [raw.image] : []), visible:raw?.visible !== false, ...(items ? {items} : {})
  } as Block;
}
...
export default function CreatePage(){
  ...
  useEffect(()=>{
    ...
    try{
      ...
      if(x.audioName)setAudioName(x.audioName); if(x.audioUrl)setAudioUrl(x.audioUrl); if(x.customBg)setCustomBg(x.customBg); if(x.customBgOpacity!==undefined)setCustomBgOpacity(Number(x.customBgOpacity)); if(x.customBgScale!==undefined)setCustomBgScale(Number(x.customBgScale)); if(x.customBgPositionX!==undefined)setCustomBgPositionX(Number(x.customBgPositionX)); if(x.customBgPositionY!==undefined)setCustomBgPositionY(Number(x.customBgPositionY));
  ...
  function projectData(){return {blocks:blocks.map(normalizeBlock),theme,background,globalFont,globalTextColor,globalCardOpacity,globalRadius,globalSpacing,globalMotion,audioName,audioUrl,customBg,customBgOpacity,customBgScale,customBgPositionX,customBgPositionY,backgroundBaseColor,bgColor1,bgColor2,bgColor3,bgColor4,backgroundOverlay}}
  ...
  function deleteDraft(){if(!window.confirm("Delete this draft?\n\nThis action cannot be undone."))return;localStorage.removeItem("hanora-project");localStorage.removeItem("cherivo-project");window.location.href="/"}
  ...
  async function uploadAudio(e:ChangeEvent<HTMLInputElement>){const file=e.target.files?.[0];if(!file)return;if(file.size>10_000_000){notify("Audio must be under 10MB");e.target.value="";return}const header=new Uint8Array(await file.slice(0,32).arrayBuffer());const isValid=(header[0]===0x49&&header[1]===0x44&&header[2]===0x33)||(header[0]===0xff&&(header[1]&0xe0)===0xe0);if(file.type!=="audio/mpeg"||!file.name.toLowerCase().endsWith(".mp3")||!isValid){notify("Choose a valid MP3 audio file");e.target.value="";return}setMediaUploading(true);try{const body=new FormData();body.append("file",file);body.append("kind","audio");const response=await fetch("/api/media",{method:"POST",body});const data=await response.json();if(!response.ok)throw new Error(data.error||"Media upload failed");setAudioUrl(data.media);setAudioPreviewUrl(data.previewUrl||"");setAudioError("");setAudioName(file.name);notify("Music added ✨")}catch(error){notify(error instanceof Error?error.message:"Media upload failed")}finally{setMediaUploading(false);e.target.value=""}}
  async function uploadMemoryVideo(e:ChangeEvent<HTMLInputElement>){const file=e.target.files?.[0];if(!file)return;if(file.size>20_000_000){notify("Video is too large. Please choose a video under 20 MB.");e.target.value="";return}const allowed=new Set(["video/mp4","video/webm","video/quicktime"]);if(!allowed.has(file.type)){notify("Unsupported video type. Choose an MP4, WebM, or MOV video.");e.target.value="";return}setMediaUploading(true);try{const body=new FormData();body.append("file",file);body.append("kind","memory-video");const response=await fetch("/api/media",{method:"POST",body});const data=await response.json();if(!response.ok)throw new Error(data.error||"Memory video upload failed");updateCurrent({memoryVideo:data.media});setMemoryVideoPreview(prev=>({...prev,[current.id]:data.previewUrl||""}));notify("Memory video added ✨")}catch(error){notify(error instanceof Error?error.message:"Memory video upload failed")}finally{setMediaUploading(false);e.target.value=""}}
  ...
  function imageAdjustment(index:number):ImageAdjustment{return current.imageAdjustments?.[String(index)] ?? {scale:100,x:50,y:50}}
  function updateImageAdjustment(index:number,patch:Partial<ImageAdjustment>){const next={...(current.imageAdjustments??{}),[String(index)]:{...imageAdjustment(index),...patch}};updateCurrent({imageAdjustments:next})}
  function resetImageAdjustment(index:number){updateImageAdjustment(index,{scale:100,x:50,y:50})}
  ...
  function sectionView(block:Block){
    const style={fontFamily:fontFamily(block.font),"--title-font":fontFamily(block.titleFont),"--subtitle-font":fontFamily(block.subtitleFont),"--heading-font":fontFamily(block.headingFont),"--body-font":fontFamily(block.bodyFont),"--local":block.accent,"--heading-size":`${block.headingSize}px`,"--body-size":`${block.bodySize}px`,"--line-height":block.lineHeight,"--letter-spacing":`${block.letterSpacing}px`,"--card-radius":`${block.radius}px`,"--story-spacing":`${globalSpacing}px`,"--global-text":globalTextColor,"--heading-color":block.headingColor,"--subtitle-color":block.subtitleColor,"--body-color":block.bodyColor,"--emoji-color":block.emojiColor,"--card-color":block.cardColor,"--section-card-opacity":`${block.cardOpacity}%`,"--letter-color":block.letterColor,"--letter-size":`${block.letterSize ?? 17}px`,"--letter-line-height":block.letterLineHeight ?? 1.8,"--letter-align":block.letterAlign ?? "left"} as CSSProperties;
      const nav=<>{block.memoryVideo&&<video className="memoryVideoPreview" src={typeof block.memoryVideo==="string"?block.memoryVideo:(memoryVideoPreview[block.id]||"")} controls playsInline preload="metadata"/>}<div className="actions"><button type="button" className="btn" onClick={()=>setScene(Math.max(0,scene-1))}><ArrowLeft size={16}/> Back</button>{scene<visible.length-1?<button type="button" className="btn primary" onClick={()=>setScene(scene+1)}>Keep going <ArrowRight size={16}/></button>:<button type="button" className="btn primary" onClick={()=>setScene(0)}><RotateCcw size={16}/> Replay</button>}</div></>;
    const edit=<button type="button" className="previewEdit" onClick={editShown}><Pencil size={13}/> Edit this section</button>;
  ...
  return <main className="creator">
    ...
    <div className="creatorGrid">
      ...
      <section className={`preview preview-${previewDevice} bg-${background} theme-${theme} motion-${globalMotion}`} style={{"--card-opacity":globalCardOpacity/100,"--page-bg":backgroundBaseColor,"--bg1":bgColor1,"--bg2":bgColor2,"--bg3":bgColor3,"--bg4":bgColor4,"--bg-overlay":backgroundOverlay/100} as CSSProperties}>
        {customBg&&<><div className="customBgLayer" style={{backgroundImage:`url("${customBg}")`,opacity:customBgOpacity/100,backgroundSize:`${customBgScale}%`,backgroundPosition:`${customBgPositionX}% ${customBgPositionY}%`}}/><div className="backgroundOverlayLayer" style={{opacity:backgroundOverlay/100}}/></>}
        {customBg&&<div className="backgroundOverlayLayer" style={{opacity:backgroundOverlay/100}}/>}
        {audioPreviewUrl&&<div className="previewAudio"><audio controls loop src={audioPreviewUrl} onCanPlay={()=>setAudioError("")} onError={()=>setAudioError("Uploaded audio could not be played.")}/>{audioError&&<span>{audioError}</span>}</div>}
          <div className="previewToolbar"><div className="deviceToggle"><button type="button" className={previewDevice==="mobile"?"active":""} onClick={()=>setPreviewDevice("mobile")}>Mobile</button><button type="button" className={previewDevice==="desktop"?"active":""} onClick={()=>setPreviewDevice("desktop")}>Desktop</button></div></div><div className="previewHint">Tap, swipe or use the button to continue ✨</div><div className="progress">{visible.map((b,i)=><i className={i<=scene?"on":""} key={b.id}/>)}</div>{shown?sectionView(shown):<div className="sceneInner"><h1>No visible sections</h1></div>}</section>
      {!previewOnly&&<aside className="sidePanel editor"><div className="editorHead"><div><h2>Edit anything</h2><small>Selected: {current.title}</small></div><span>{selected+1}/{blocks.length}</span></div>
        ...
        {mediaUploading&&<div className="mediaUploadStatus">Uploading music…</div>}
        ...
        {(current.type==="gallery"||current.type==="memories")&&(current.images?.length??0)>0&&<div className="imageAdjustmentEditor"><div className="divider">Photo framing</div><label>Photo<select value={selectedGalleryImage} onChange={e=>setSelectedGalleryImage(Number(e.target.value))}>{(current.images??[]).map((_,i)=><option key={i} value={i}>Photo {i+1}</option>)}</select></label><label>Zoom <strong>{imageAdjustment(selectedGalleryImage).scale}%</strong><input type="range" min="100" max="220" value={imageAdjustment(selectedGalleryImage).scale} onChange={e=>updateImageAdjustment(selectedGalleryImage,{scale:Number(e.target.value)})}/></label><label>Horizontal position <strong>{imageAdjustment(selectedGalleryImage).x}%</strong><input type="range" min="0" max="100" value={imageAdjustment(selectedGalleryImage).x} onChange={e=>updateImageAdjustment(selectedGalleryImage,{x:Number(e.target.value)})}/></label><label>Vertical position <strong>{imageAdjustment(selectedGalleryImage).y}%</strong><input type="range" min="0" max="100" value={imageAdjustment(selectedGalleryImage).y} onChange={e=>updateImageAdjustment(selectedGalleryImage,{y:Number(e.target.value)})}/></label><button type="button" className="btn" onClick={()=>resetImageAdjustment(selectedGalleryImage)}>Reset photo framing</button></div>}
        {current.type!=="gallery"&&current.type!=="memories"&&<div className="heroPhotoEditor">
        ...
        <div className="memoryVideoEditor"><div className="divider">Memory Video</div><p className="helperText">Optional personal video for this page. It is shown inside the greeting, never as a background.</p><label>Upload video<input type="file" accept="video/mp4,video/webm,video/quicktime,.mp4,.webm,.mov" onChange={uploadMemoryVideo} disabled={mediaUploading}/></label>{current.memoryVideo&&<><video className="memoryVideoPreview" src={typeof current.memoryVideo==="string"?current.memoryVideo:(memoryVideoPreview[current.id]||"")} controls playsInline preload="metadata"/><button type="button" className="btn danger small full" onClick={()=>{updateCurrent({memoryVideo:undefined});setMemoryVideoPreview(prev=>{const next={...prev};delete next[current.id];return next})}}>Remove memory video</button></>}</div>
        {customBg&&<div className="backgroundPositionEditor"><div className="divider">Background photo position</div><label>Photo zoom <strong>{customBgScale}%</strong><input type="range" min="100" max="240" value={customBgScale} onChange={e=>setCustomBgScale(Number(e.target.value))}/></label><label>Horizontal position <strong>{customBgPositionX}%</strong><input type="range" min="0" max="100" value={customBgPositionX} onChange={e=>setCustomBgPositionX(Number(e.target.value))}/></label><label>Vertical position <strong>{customBgPositionY}%</strong><input type="range" min="0" max="100" value={customBgPositionY} onChange={e=>setCustomBgPositionY(Number(e.target.value))}/></label><button type="button" className="btn" onClick={()=>{setCustomBgScale(100);setCustomBgPositionX(50);setCustomBgPositionY(50)}}>Reset position</button></div>}
        ...
        <div className="divider">🎵 Music</div><p className="helperText">Upload one MP3 file for background music.</p><label>Upload MP3<input type="file" accept="audio/mpeg,.mp3" onChange={uploadAudio} disabled={mediaUploading}/></label>{audioUrl&&<div className="audioControlCard"><b>🎵 {audioName||"Music"}</b><button type="button" className="btn small" onClick={()=>{setAudioName("");setAudioUrl("");setAudioPreviewUrl("")}}>Remove music</button></div>}
        {current.type==="letter"&&<div className="letterControls"><div className="divider">Letter styling</div><div className="two"><label>Letter text colour<input className="color" type="color" value={current.letterColor ?? "#2d2024"} onChange={e=>updateCurrent({letterColor:e.target.value})}/></label><label>Letter font<select value={current.bodyFont ?? "serif"} onChange={e=>updateCurrent({bodyFont:e.target.value as FontName})}><option value="serif">Playfair Display</option><option value="sans">DM Sans</option><option value="script">Great Vibes</option><option value="caveat">Caveat</option></select></label></div><div className="two"><label>Letter size <strong>{current.letterSize ?? 17}px</strong><input type="range" min="12" max="30" value={current.letterSize ?? 17} onChange={e=>updateCurrent({letterSize:Number(e.target.value)})}/></label><label>Line spacing <strong>{Number(current.letterLineHeight ?? 1.8).toFixed(2)}</strong><input type="range" min="1" max="2.5" step=".05" value={current.letterLineHeight ?? 1.8} onChange={e=>updateCurrent({letterLineHeight:Number(e.target.value)})}/></label></div><label>Letter alignment<select value={current.letterAlign ?? "left"} onChange={e=>updateCurrent({letterAlign:e.target.value as Block["letterAlign"]})}><option value="left">Left</option><option value="center">Center</option><option value="right">Right</option></select></label></div>}
        <div className="draftActions"><button type="button" className="btn" onClick={save}>Save changes</button><button type="button" className="btn danger" onClick={deleteDraft}>Delete draft</button></div>
      </aside>}
    </div>
